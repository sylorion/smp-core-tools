...
async function entityCreator(entityContext, inputs, appContext) {
    throw new Error('Deleted function');
}
...
export async function saveAndPublishEntity(entityContext, inputs, appContext) {
  ...
  try {
    ...
    if(entityContext.entityCommitCallBackFn){
      newEntity = await entityContext.entityCommitCallBackFn(newEntity, dbOptions);
      if (!newEntity) {
    ...
    if (entityContext.entityPublisherFn) {
      await entityContext.entityPublisherFn(appContext, entityContext, newEntity);
    }
